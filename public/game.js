...
const RARITIES=[
 {key:"normal",name:"Normal",mult:1,chance:44.5},
 {key:"rare",name:"Rare",mult:1.35,chance:30},
 {key:"epic",name:"Epic",mult:1.8,chance:15},
 {key:"mythic",name:"Mythic",mult:2.5,chance:6},
 {key:"legendary",name:"Legendary",mult:3.5,chance:3},
 {key:"imperial",name:"Imperial",mult:4.7,chance:1},
 {key:"celestial",name:"Celestial",mult:6.2,chance:.4},
 {key:"eternal",name:"Eternal",mult:8,chance:.1}
];
const ZONES=[
...
// V22.88: kisebb alappet-bónuszok a kompakt Wave 400-as balanszhoz.
PET_POOL.forEach(p=>p.value=Math.max(.002,Number(p.value||0)*.20));
const DUNGEONS=[
...
function normalizeV6Save(s){
 ...
 if(!s.petMountBalanceV288){s.pets.forEach(p=>{if(!p||typeof p!=="object")return;p.value=Math.max(0,Number(p.value||0)*.35);if(Array.isArray(p.extraOptions))p.extraOptions.forEach(o=>o.value=Math.max(0,Number(o?.value||0)*.35))});s.petMountBalanceV288=true}
 if(!s.petBalanceV289){s.pets.forEach(p=>{if(!p||typeof p!=="object")return;p.value=Math.max(0,Number(p.value||0)*(4/7));if(Array.isArray(p.extraOptions))p.extraOptions.forEach(o=>o.value=Math.max(0,Number(o?.value||0)*(4/7)))});s.petBalanceV289=true}
 ...
 s.dungeonBatchUnlocked=Boolean(s.dungeonBatchUnlocked);
...
function upgradeMult(item){return 1+item.plus*.14}
...
function petEffectiveBonusV288(type,raw){const caps={damage:.65,gold:.45,crit:.18,drop:.30},cap=caps[type]||.35,v=Math.max(0,Number(raw||0));return cap*v/(v+cap)}
function bonuses(){
 ...
 petDamage=petEffectiveBonusV288("damage",petDamage);gold=petEffectiveBonusV288("gold",gold);crit=petEffectiveBonusV288("crit",crit);drop=petEffectiveBonusV288("drop",drop);
 atk*=1+petDamage;
 return {atk,def,gold,crit,drop}
}
function damageCoreV281(bossContext=false){
...
const goldBonusBaseV23196=goldBonus;
goldBonus=function(){
 const base=Math.max(1,Number(goldBonusBaseV23196()||1));
 const io=typeof itemOptionBonuses==="function"?itemOptionBonuses():{};
 return base*(1+Math.max(0,Number(io.gold||0))/100)*(typeof prestigeGoldMultiplierV23201==="function"?prestigeGoldMultiplierV23201():1);
};
...
function dropBonus(){let io=itemOptionBonuses();return skillBonus("drop")+bonuses().drop+(save.base.luck-1)*.01+save.paragonStats.drop*.01*save.paragonLevel+io.drop/100+(typeof prestigeDropBonusV23201==="function"?prestigeDropBonusV23201():0)}
...
function power(){
 const raw=Math.max(0,Number(rawPowerV280()||0)),cap=100000;
 return Math.max(0,Math.min(cap,Math.floor(cap*raw/(raw+cap*.22))));
}
...
function baseCost(d){return Math.floor(d.base*Math.pow(1.255,save.base[d.key]-1))}
function rarityRoll(){
 const bonus=Math.max(0,dropBonus()*100),r=Math.random()*100;
 const eternal=.10+Math.min(.10,bonus*.002);
 const celestial=.40+Math.min(.30,bonus*.005);
 const imperial=1.00+Math.min(.60,bonus*.010);
 const legendary=3.00+Math.min(1.20,bonus*.018);
 const mythic=6.00+Math.min(2.00,bonus*.025);
 const epic=15.00+Math.min(3.00,bonus*.035);
 const rare=30.00+Math.min(4.00,bonus*.045);
 let x=eternal;if(r<x)return RARITIES.find(v=>v.key==="eternal");
 x+=celestial;if(r<x)return RARITIES.find(v=>v.key==="celestial");
 x+=imperial;if(r<x)return RARITIES.find(v=>v.key==="imperial");
 x+=legendary;if(r<x)return RARITIES.find(v=>v.key==="legendary");
 x+=mythic;if(r<x)return RARITIES.find(v=>v.key==="mythic");
 x+=epic;if(r<x)return RARITIES.find(v=>v.key==="epic");
 x+=rare;if(r<x)return RARITIES.find(v=>v.key==="rare");
 return RARITIES.find(v=>v.key==="normal");
}

const ITEM_OPT_DEFS={
 atkPct:{name:"Sebzés %",desc:"A teljes sebzésedet növeli.",unit:"%",min:1,max:16},
 hpPct:{name:"Max HP %",desc:"A maximális életerődet növeli.",unit:"%",min:1,max:18},
 defPct:{name:"Védelem %",desc:"A teljes védelmedet növeli.",unit:"%",min:1,max:16},
 crit:{name:"Krit esély",desc:"Növeli a kritikus találat esélyét.",unit:"%",min:.5,max:7},
 drop:{name:"Drop bónusz",desc:"Növeli a felszerelés dobási esélyét.",unit:"%",min:.5,max:9},
 gold:{name:"Arany bónusz",desc:"Növeli a megszerzett aranyat.",unit:"%",min:2,max:18},
 hpRegen:{name:"HP Regen",desc:"Másodpercenként extra HP%-ot regenerálsz.",unit:"%/mp",min:.1,max:1.8},
 bossDmg:{name:"Boss sebzés",desc:"Növeli a Bossok ellen okozott sebzést.",unit:"%",min:2,max:18},
 pvpDmg:{name:"PvP sebzés",desc:"Növeli a játékosok elleni sebzést.",unit:"%",min:1,max:10}
};
function rarityOptCount(rarity){
 return {normal:1,rare:2,epic:3,mythic:4,legendary:5,imperial:5,celestial:5,eternal:5}[rarity]||1;
}
function optionScaleByRarity(rarity){
 return {normal:.55,rare:.72,epic:.88,mythic:1,legendary:1.18,imperial:1.32,celestial:1.48,eternal:1.65}[rarity]||.55;
}
function ensureItemOptions(it){
 if(!it)return it;
 if(!Array.isArray(it.options))it.options=[];
 it.gold=Math.max(0,Number(it.gold||0));
...
function itemRerollCost(it){
 const r={normal:1,rare:2,epic:4,mythic:8,legendary:15,imperial:22,celestial:30,eternal:42}[it.rarity]||1;
...
function itemOptionBonuses(){
 const out={atkPct:0,hpPct:0,defPct:0,crit:0,drop:0,gold:0,hpRegen:0,bossDmg:0,pvpDmg:0};
...
function addItem(it){
 ...
 if(save.inventory.length>=(typeof inventoryLimitV23201==="function"?inventoryLimitV23201():120) && Number(save.highestZoneEver||save.zone||0)>=3){save.gold+=sellValue(it);return toast("🎒 Inventory tele — tárgy automatikusan eladva.")}
...
function kill(){
 let z=ZONES[save.zone],g=Math.floor(zoneMobGold(save.zone)*paragonOverfarmV274().rewardMult);
 save.gold+=g;save.stats.goldEarned+=g;save.xp+=Math.floor(z.xp*(typeof xpRewardMultiplierV23201==="function"?xpRewardMultiplierV23201():1));save.kills++;
 const rewardMult=paragonOverfarmV274().rewardMult;
 if(Math.random()<(.07+save.base.mining*.005)*rewardMult)save.ore++;
 if(Math.random()<(.007+dropBonus()*.05)*rewardMult)save.soul++;
 if(Math.random()<.006*rewardMult)save.tickets++;
 if(Math.random()<(z.drop+dropBonus())*paragonOverfarmV274().rewardMult){addItem(createItem());addFarmActivityV264("drops",1)}
...
function zoneMinPowerV319(zone){
 const gates=[0,650,1400,2600,4500,7000,10500,15000,21000];
...
function progressionMinimumZoneV260(){
 const w=Math.max(1,Number(save?.wave||1)),steps=zoneWaveThresholdsV285();
 let byWave=0;
 steps.forEach((n,i)=>{if(w>=n)byWave=i});
 let p=0;
 try{p=Math.max(0,Number(typeof power==="function"?power():0)||0)}
 catch(e){console.warn("[SAFE POWER GATE]",e);return Math.max(0,Math.min(ZONES.length-1,Number(save?.zone||0)))}
 let byPower=0;
...
function itemPowerScore(it){
 ...
 const rarityScore={normal:0,rare:100,epic:250,mythic:500,legendary:900,imperial:1500,celestial:2400,eternal:3600}[it.rarity]||0;
 score+=rarityScore;

 const optWeight={atkPct:70,hpPct:22,defPct:48,crit:58,drop:12,gold:10,hpRegen:25,bossDmg:40,pvpDmg:12};
...
function buyNpcOfferV246(id){
 ...
 if(o.type==="gear"&&save.inventory.length>=(typeof inventoryLimitV23201==="function"?inventoryLimitV23201():120))return toast("🎒 Az inventory megtelt.");
...
function petEquipScore(p){
 ...
 return petBonusEntries(p).reduce((sum,x)=>sum+petEffectiveBonusV288(x.bonus==="all"?"damage":x.bonus,Math.max(0,Number(x.value||0)))*(weights[x.bonus]||1),0)+(rarity[p.rarity]||0);
}
...
function renderPets(){
 ...
 const summonAmount=Math.max(1,Math.floor(Number($("#petSummonAmount")?.value||1))),summonBtn=$("#petSummon");if(summonBtn){summonBtn.textContent=`🎲 ${summonAmount} PET · ${fmt(eco.petSummonCost*summonAmount)} 💎`;summonBtn.disabled=save.gems<eco.petSummonCost*summonAmount}
...
function summonPet(){const eco=economyCfg(),amount=Math.max(1,Math.min(50,Math.floor(Number($("#petSummonAmount")?.value||1)))),cost=eco.petSummonCost*amount;if(save.gems<cost)return toast(`Nincs elég gyémánt. ${amount} pet idézési ára ${fmt(cost)} 💎.`);save.gems-=cost;const rates=eco.petSummonRates,order=["normal","rare","epic","mythic","legendary"],weights=order.map(k=>Math.max(0,Number(rates[k]||0))),total=weights.reduce((a,b)=>a+b,0)||1,won={normal:0,rare:0,epic:0,mythic:0,legendary:0};for(let summon=0;summon<amount;summon++){let roll=Math.random()*total,rarity="normal";for(let i=0;i<order.length;i++){roll-=weights[i];if(roll<=0){rarity=order[i];break}}const pool=PET_POOL.filter(p=>p.rarity===rarity),p=pool[Math.floor(Math.random()*pool.length)]||PET_POOL[0];save.pets.push({...p,fusionRarity:"common",fusionLevel:0,fusionMultiplier:1});won[rarity]++}persist();renderAll();const summary=order.filter(k=>won[k]).map(k=>`${rarityName(k)} ${won[k]}×`).join(" · ");toast(`🐾 ${amount} pet megidézve · ${summary} · -${fmt(cost)} 💎`)}
...
function finalZoneParagonStateV23195(){
 try{
   const last=Math.max(0,ZONES.length-1);
   const g=typeof zoneGoalStateV23194==="function"
     ?zoneGoalStateV23194(last)
     :{waveDone:false,powerDone:false,waveNeed:paragonWaveRequirement(),powerNeed:0,waveNow:Number(save?.wave||1),powerNow:0};
   const onFinalZone=Number(save?.zone||0)>=last;
   return {
     last,
     onFinalZone,
     waveDone:Boolean(g.waveDone),
     powerDone:Boolean(g.powerDone),
     waveNeed:Number(g.waveNeed||paragonWaveRequirement()),
     powerNeed:Number(g.powerNeed||0),
     waveNow:Number(g.waveNow||save?.wave||1),
     powerNow:Number(g.powerNow||0),
     eligible:Boolean(onFinalZone&&g.waveDone&&g.powerDone)
   };
 }catch(e){
   return {last:Math.max(0,ZONES.length-1),onFinalZone:false,waveDone:false,powerDone:false,waveNeed:paragonWaveRequirement(),powerNeed:0,waveNow:Number(save?.wave||1),powerNow:0,eligible:false};
 }
}


/* ================= V23.20.1 PRESTIGE ENDGAME SHOP ================= */
const PRESTIGE_SHOP_V23201=[
 {id:"tickets10",icon:"🎟️",name:"Dungeon Ticket csomag",type:"Fogyóeszköz",cost:2,desc:"+10 Dungeon jegy"},
 {id:"gems300",icon:"💎",name:"Gyémánt láda",type:"Fogyóeszköz",cost:3,desc:"+300 gyémánt"},
 {id:"ore250",icon:"⛏️",name:"Érc láda",type:"Fogyóeszköz",cost:3,desc:"+250 érc"},
 {id:"soul120",icon:"🧿",name:"Lélekkő láda",type:"Fogyóeszköz",cost:4,desc:"+120 lélekkő"},
 {id:"pet10",icon:"🐾",name:"Pet Summon ×10",type:"Fogyóeszköz",cost:5,desc:"10 pet idézés gyémánt nélkül"},
 {id:"gearLegend",icon:"🎁",name:"Legendary+ Gear láda",type:"Fogyóeszköz",cost:8,desc:"1 garantált Legendary vagy jobb tárgy"},
 {id:"drop2h",icon:"🍀",name:"Drop Booster",type:"2 órás",cost:5,desc:"+20% felszerelés drop 2 órára"},
 {id:"gold2h",icon:"💰",name:"Arany Booster",type:"2 órás",cost:5,desc:"+25% farm/boss arany 2 órára"},
 {id:"dungeon2h",icon:"🏰",name:"Dungeon Booster",type:"2 órás",cost:8,desc:"+20% dungeon erő 2 órára"},
 {id:"inv50",icon:"🎒",name:"Inventory +50",type:"Végleges",cost:10,desc:"+50 végleges tárhely",once:true},
...
function prestigeShopStateV23201(){
 ...
 save.prestigeShopV23201.boosts=save.prestigeShopV23201.boosts||{};
 return save.prestigeShopV23201;
}
function prestigeBoostActiveV23201(id){return Number(prestigeShopStateV23201().boosts[id]||0)>Date.now()}
function prestigeBoostRemainingV23201(id){return Math.max(0,Number(prestigeShopStateV23201().boosts[id]||0)-Date.now())}
function activeAuraBonusV23201(key){
...
function prestigeShopPetV23201(count=10){
 ...
 for(let n=0;n<Math.max(1,Math.floor(Number(count||10)));n++){
  ...
  save.pets.push({...p,fusionRarity:"common",fusionLevel:0,fusionMultiplier:1});
 }
}
function prestigeGearCrateV23201(){
 let it=createItem();
 const roll=Math.random()*100;let rar=roll<72?"legendary":roll<91?"imperial":roll<98?"celestial":"eternal";
 const rd=RARITIES.find(x=>x.key===rar)||RARITIES.find(x=>x.key==="legendary");
 it.rarity=rar;it.name=`${rd?.name||rarityName(rar)} ${SLOT_NAMES[it.slot]}`;
 const scale=(1+save.zone*.75)*(1+Math.min(500,save.wave)*.004)*(rd?.mult||3.5);
 if(it.slot==="weapon")it.atk=Math.max(10,Math.min(itemRawAttackCapV290(it)-1,Math.floor(itemRawAttackCapV290(it)*(.48+Math.random()*.38))));
 if(it.slot==="armor"||it.slot==="helmet")it.def=Math.max(5,Math.floor(2.25*scale));
 if(it.slot==="gloves")it.crit=Math.max(.01,.005*scale);
 if(it.slot==="boots")it.def=Math.max(3,Math.floor(1.2*scale));
 if(it.slot==="ring")it.drop=Math.max(.02,.012*scale);
...
window.renderPrestigeTokenShopV23201=renderPrestigeTokenShopV23201;

function renderParagon(){
 const req=paragonWaveRequirement();
 const finalGateV23195=finalZoneParagonStateV23195();
 const eligible=finalGateV23195.eligible;
 ...
 if(text){
   const f=finalGateV23195;
   const missing=[];
   if(!f.onFinalZone)missing.push("érd el a Csiki fajzat utolsó farmterületét");
   if(!f.waveDone)missing.push(`Wave ${fmt(f.waveNeed)}`);
   if(!f.powerDone)missing.push(`${fmt(Math.max(0,f.powerNeed-f.powerNow))} hiányzó ERŐ`);
   text.textContent=(eligible
     ?`🌟 Paragon szintlépés elérhető a Csiki fajzat területén!`
     :`Paragonhoz: ${missing.join(" · ")}.`)+` · Paragon statok hatásszorzója: ${paragonScale}×`;
 }

 const btn=$("#prestigeBtn");
 if(btn){
   btn.disabled=!eligible;
   btn.textContent=eligible?`🌟 PARAGON SZINTLÉPÉS`:`🌟 PARAGON · CIKI FAJZAT CÉL`;
 }
...
function doPrestige(automatic=false){
 ...
 const f=finalZoneParagonStateV23195();
 if(!f.eligible){
   if(!f.onFinalZone)return toast("🔒 Paragonhoz előbb el kell érned a Csiki fajzat utolsó farmterületét.");
   if(!f.waveDone)return toast(`🌊 Paragonhoz még a Wave ${fmt(f.waveNeed)} célt kell teljesítened.`);
   if(!f.powerDone)return toast(`⚔️ Paragonhoz még ${fmt(Math.max(0,f.powerNeed-f.powerNow))} ERŐ hiányzik.`);
...
$("#petSummonAmount")?.addEventListener("change",renderPets);
...
if(away>15){
 ...
 if(kills>0){let g=Math.floor(kills*zoneMobGold(save.zone)*paragonOverfarmV274().rewardMult);save.gold+=g;save.stats.goldEarned+=g;save.kills+=kills;save.xp+=Math.floor(kills*z.xp*(typeof xpRewardMultiplierV23201==="function"?xpRewardMultiplierV23201():1));toast(`🌙 Offline farm: ${fmt(kills)} kill · ${fmt(g)} arany`)}
}
...
function normalEnemyMaxHp(){
 ...
 const wave=Math.max(1,Number(save.wave||1)),prestige=Math.max(0,Number(save.prestigeLevel||0));
 const zoneBase=[120,380,950,2200,4700,8500,14000,22000,32000][zone]||32000;
 const thresholds=zoneWaveThresholdsV285();
...
function addFarmActivityV264(){}
function farmActivityPointsV264(){return 0}
...
function passFarmCheckpointV262(){return true}
...
function v10BossMaxHp(){
 const mob=Math.max(10,normalEnemyMaxHp()),zone=Math.max(0,Number(save.zone||0)),wave=Math.max(1,Number(save.wave||1));
 const progress=Math.min(1,wave/Math.max(1,paragonWaveRequirement()));
 const mult=4.5+zone*.32+progress*1.1;
 return Math.max(mob,Math.floor(mob*mult));
}
...
function v10AwardNormalKill(){
 ...
 casinoFarmGoldV256(g);save.stats.goldEarned+=g;save.xp+=Math.floor(z.xp*(typeof xpRewardMultiplierV23201==="function"?xpRewardMultiplierV23201():1));save.kills++;
...
function v10AwardBossKill(){
 ...
 const bossGemsWon=bossGemAmount>0&&Math.random()*100<bossGemChance*overfarm.rewardMult?bossGemAmount:0;
 casinoFarmGoldV256(reward);save.stats.goldEarned+=reward;save.gems+=bossGemsWon;if(Math.random()<overfarm.rewardMult)save.soul++;save.stats.bosses++;addFarmActivityV264("bosses",1);
 if(Math.random()<.80*overfarm.rewardMult)addItem(createItem());
...
function v10Render(){
 ...
 if($("#combatRecommendedDefense"))$("#combatRecommendedDefense").textContent=fmt(recommendedDefenseV271());
...
function v11ApplyContent(){
 ...
 (c.pets||[]).forEach((p,i)=>{
   ...
   PET_POOL.push({_customId:p.id||`pet-${i}`,name:p.name||"Pet",icon:p.icon||"🐾",bonus:p.bonus||"damage",value:Number(p.value??p.damageBonus??5)/100*.20,rarity:p.rarity||"rare"});
 });
 // Custom auras
...
v10RawEnemyDamage=function(){
 ...
 let raw=Math.max(configured,scaled)*Math.max(.5,Number(V10CFG.bossDamageMult||1.45)/1.45)*defensePressureV271();
 return Math.max(1,Math.floor(raw));
};
...
v10AwardBossKill=function(){
 ...
 const overfarm=paragonOverfarmV274(),reward=Math.floor(bossGoldReward(b.gold||0)*overfarm.rewardMult);
 ...
 const gemsWon=bossGems>0&&Math.random()*100<gemChance*overfarm.rewardMult?bossGems:0;
 save.gems+=gemsWon;save.soul+=Math.floor(Number(b.soul||0)*overfarm.rewardMult);save.stats.bosses++;addFarmActivityV264("bosses",1);
 while(save.xp>=needXp()){save.xp-=needXp();save.level++}
 if(Math.random()<(Number(b.dropChance||80)/100)*overfarm.rewardMult)addItem(createItem());
...
(function(){
  ...
  const V2241_DUNGEON_BALANCE=[
    [900,[1200,1800]],[1800,[2500,3800]],[3200,[5000,7500]],[5000,[9000,13500]],[7500,[15000,22000]],
    [10500,[24000,35000]],[14000,[36000,52000]],[18000,[52000,76000]],[23000,[76000,110000]],[30000,[115000,165000]]
  ];
  ...
  function p218(){const p=(typeof power==="function")?Number(power()||0):0;return p*(typeof prestigeDungeonMultiplierV23201==="function"?prestigeDungeonMultiplierV23201():1)}
  ...
  function ensureDungeonState(){
    ...
    if(!s.dungeonBatchV278) s.dungeonBatchV278={};
  }
  ...
  function renderDungeonV218(){
        ...
        ${DUNGEONS_V218.map(d=>{
          ...
          <article class="v218-dungeon-card ${d.safe?"safe":""} theme-${d.visualTheme||"cave"}">
            ...
            <div class="v218-dungeon-stats">
              ...
              <span><small>Ajánlott DEF</small><b>${fmt218(typeof dungeonRecommendedDefenseV271==="function"?dungeonRecommendedDefenseV271(d):0)}</b></span>
              <span><small>Jegy / futam</small><b>${d.ticketCost}</b></span>
    ...
    root.querySelectorAll("[data-v278-premium-shop]").forEach(btn=>btn.onclick=()=>document.querySelector('[data-tab="shop"]')?.click());
  }
...
(function(){
    ...
    panel.innerHTML=`
      <div class="v219-battle-title">
        <div>
          ...
          <h3>${b.dungeon.name} ${b.batchCount>1?`· ${b.batchCount}× SOROZAT`:""}</h3>
        </div>
      ...
      <div class="v220-arena theme-${b.dungeon.visualTheme||"cave"}">
        ...
        <div class="v219-fighter player">
          ...
          <div class="v219-fighter-main">
            ...
            <small>⚔️ Erő: ${F(P())} · 🛡️ DEF: ${F(typeof v10Defense==="function"?v10Defense():0)} · 🧱 ${typeof defenseReductionV265==="function"?(defenseReductionV265()*100).toFixed(1):0}% · ✨ Blokk: ${typeof defenseGuardChanceV265==="function"?(defenseGuardChanceV265()*100).toFixed(1):0}%</small>
          </div>
        </div>
  ...
  function finishBattle(win){
    ...
    const batch=Math.max(1,Number(b.batchCount||1)),wins=Math.max(0,Math.min(batch,Number(b.batchWins??(win?1:0)))),losses=batch-wins;
    if(wins>0){
      const rw={gold:0,ore:0,gems:0,soul:0};
      for(let rewardRun=0;rewardRun<wins;rewardRun++){
       const one=(function(){
        const r=d.rewards||{};
       ...
       Object.keys(rw).forEach(k=>rw[k]+=Number(one[k]||0));
      }

      s.dungeonStats.wins+=wins;s.dungeonStats.losses+=losses;
      s.stats=s.stats||{};s.stats.dungeons=Number(s.stats.dungeons||0)+wins;addFarmActivityV264("dungeons",wins);
      s.dungeonStats.streak=losses?0:Number(s.dungeonStats.streak||0)+wins;
      s.dungeonClears[d.id]=Number(s.dungeonClears[d.id]||0)+wins;
      ...
      s.dungeonStats.streak=0;
      b.playerHp=0;
      b.log=`💀 ${batch} futamból egyik sem sikerült. ${batch*Number(d.ticketCost||1)} jegy elfogyott, de teljes HP-val éledsz újra.`;
      drawBattle();

      const mh=maxPlayerHp();
      s.playerHp=mh;
      if(typeof toast==="function")toast(`💀 ${d.name}: 0/${batch} siker.`);
    }
  ...
  function startBattle(d,batchCount=1){
    const s=S();
    if(!s.dungeonStats)s.dungeonStats={runs:0,wins:0,losses:0,streak:0};
    if(!s.dungeonClears)s.dungeonClears={};
    ...
    s.tickets=tickets-totalCost;
    s.dungeonStats.runs+=batchCount;

    const chance=(function(){
      if(d.safe)return 1;
      if(typeof damageCoreV281==="function"&&Number(damageCoreV281(true)||0)>=bossMaxHp(d))return 1;
    ...
    battleTimer=setInterval(()=>{
      ...
      if(b.win){
        const simulated=Math.max(1,Math.floor(b.bossMaxHp/roundsTarget*(.82+Math.random()*.35))),actual=typeof damageCoreV281==="function"?Math.max(1,Math.floor(damageCoreV281(true))):simulated,bossDmg=Math.max(simulated,actual);
        const incomingBase=d.safe
          ? Math.max(1,Math.floor(b.playerMaxHp*.035))
          : Math.max(1,Math.floor(b.playerMaxHp*(.045+Math.random()*.045)));
        const incomingRoll=typeof dungeonBlockRollV272==="function"?dungeonBlockRollV272(incomingBase*(typeof dungeonIncomingMultiplierV266==="function"?dungeonIncomingMultiplierV266(d):1)):{blocked:false,damage:incomingBase},incoming=incomingRoll.damage;
        ...
        b.log=`⚔️ ${F(bossDmg)} sebzés a bossnak · ${incomingRoll.blocked?"🛡️ BLOKK! ":"☠️ "}${F(incoming)} sebzés érkezett.`; triggerDungeonEffect(b.dungeon.effectType); const battlePanel=document.getElementById("v219DungeonBattle");battlePanel?.classList.add(incomingRoll.blocked?"v272-block":"v220-hit");setTimeout(()=>battlePanel?.classList.remove("v220-hit","v272-block"),280);
        ...
        const incomingBase=b.playerMaxHp*(.11+Math.random()*.09)*(typeof dungeonIncomingMultiplierV266==="function"?dungeonIncomingMultiplierV266(d):1),incomingRoll=typeof dungeonBlockRollV272==="function"?dungeonBlockRollV272(incomingBase):{blocked:false,damage:Math.max(1,Math.floor(incomingBase))},incoming=incomingRoll.damage;
        b.bossHp=Math.max(Math.floor(b.bossMaxHp*.18),b.bossHp-bossDmg);
        b.playerHp=Math.max(0,b.playerHp-incoming);
        b.log=`⚔️ ${F(bossDmg)} sebzés a bossnak · ${incomingRoll.blocked?"🛡️ BLOKK! ":"💥 "}${F(incoming)} sebzést kaptál.`; triggerDungeonEffect(b.dungeon.effectType); const battlePanel=document.getElementById("v219DungeonBattle");battlePanel?.classList.add(incomingRoll.blocked?"v272-block":"v220-hit");setTimeout(()=>battlePanel?.classList.remove("v220-hit","v272-block"),280);
  ...
  function enhancedRender(){
    ...
    root.querySelectorAll("[data-v218-dungeon]").forEach(btn=>{
      ...
      btn.onclick=()=>{const selected=Number(S().dungeonBatchV278?.[d.id]||1);startBattle(d,selected)};
    });
...
(function(){
  ...
  function renderFusionPanel(){
    ...
    const possible=groups.reduce((sum,g)=>sum+Math.floor(g.length/required(g[0].p)),0),minimumCost=groups.length?Math.min(...groups.map(g=>craftCost(g[0].p))):0,bulk=document.createElement("button");bulk.id="fuseAllIdenticalPetsV287";bulk.className="fuse-all-identical-pets-v287";bulk.disabled=!possible||Number(S().gems||0)<minimumCost;bulk.textContent=`⚡ ÖSSZES AZONOS PET KRAFTOLÁSA · ${possible}+`;
    panel.querySelector(".v224-head")?.appendChild(bulk);
    bulk.onclick=()=>{let crafted=0,spent=0,guard=0;while(guard++<5000){const available=fusionGroups().filter(g=>Number(S().gems||0)>=craftCost(g[0].p));if(!available.length)break;const result=fuse(available[0]);if(!result)break;crafted++;spent+=result.cost}if(!crafted)return toast("Nincs elegendő azonos pet vagy gyémánt a tömeges kraftoláshoz.");persist();renderAll();toast(`⚡ ${crafted} azonos petkraft elkészült · -${fmt(spent)} 💎`);renderFusionPanel()};
...
(function(){
  // V22.80: a megszerzett pet- és sebzésfejlődés többé nem növeli vissza
  // automatikusan az ellenfél HP-ját. Így a végjátékban elérhető az együtéses farm.
})();
...
/* V23.19 STABLE POWER BALANCE ROLLBACK
...
/* V23.19.5 PARAGON FINAL-ZONE GATE
   Paragon is available only on the final Csiki fajzat farm area
   after both Wave and Power goals are completed. */
window.OMI_PARAGON_FINAL_ZONE_V23195=true;

/* V23.19.6 LONGER WAVES + EASIER POWER
   Paragon: Wave 500 + final farm-area power goal.
   Power cap: 100,000.
   High rarities restored: Imperial / Celestial / Eternal.
   Gold item option restored.
   Gear +15 scaling strengthened.
   Mob / farm boss / dungeon boss HP rebalanced around the easier power curve.
*/
window.OMI_BALANCE_V23196={
 paragonWave:500,
 maxPower:100000,
...
(function(){
 function refreshPrestigeShopV23201(){try{renderPrestigeTokenShopV23201()}catch(e){console.warn("[PRESTIGE SHOP]",e)}}
 window.addEventListener("load",()=>{setTimeout(refreshPrestigeShopV23201,350);setInterval(refreshPrestigeShopV23201,5000)});
